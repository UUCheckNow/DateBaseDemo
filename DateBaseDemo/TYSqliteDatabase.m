...
@implementation TYSqliteDatabase
 ...
 */
+(BOOL)dealSqls:(NSArray <NSString *>*)sqls withDataBaseName:(nonnull NSString *)dataBaseName{
    
    [self beginTransactionDataBaseName:dataBaseName];
    
    for (NSString *sql in sqls) {
        
        BOOL result = [self deal:sql withDataBaseName:dataBaseName];
        if (!result) {
            
            // 执行失败，进行回滚
            [self rollBackTransactionDataBaseName:dataBaseName];
    ...
    // 走到这里说明都执行成功了，进行提交
    [self commitTransactionDataBaseName:dataBaseName];
...
// 开启事务
+(void)beginTransactionDataBaseName:(NSString *)dataBaseName{
    
    [self deal:@"begin transaction" withDataBaseName:dataBaseName];
}

// 提交事务
+(void)commitTransactionDataBaseName:(NSString *)dataBaseName{
    
    [self deal:@"commit transaction" withDataBaseName:dataBaseName];
}

// 回滚事务
+(void)rollBackTransactionDataBaseName:(NSString *)dataBaseName{
    
    [self deal:@"rollback transaction" withDataBaseName:dataBaseName];
}
...
+(BOOL)openDB:(NSString *)dataBaseName{
    ...
    // 1.获取数据库的名字以及对应的路径
    // 1.1、获取数据库的名字
    NSString *sqliteName = [NSString stringWithFormat:@"ty_%@.sqlite",dataBaseName];
    // 1.2、对应的路径
